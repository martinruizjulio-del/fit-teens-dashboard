      ...
      SELECT avg(nota_alumno) FROM (
        SELECT a.id,
          (
            ...
            COALESCE(calcular_nota('cfs','rockport',         a.sexo, EXTRACT(YEAR FROM age(a.fecha_nacimiento))::int, p.rockport_min + p.rockport_seg/60.0),0)
          )::numeric / NULLIF(
            ...
            (CASE WHEN p.rockport_min IS NOT NULL THEN 1 ELSE 0 END), 0)
          AS nota_alumno
        FROM pruebas_cfs p
        JOIN alumnos a ON a.id = p.alumno_id
        JOIN grupos g ON g.id = a.grupo_id
